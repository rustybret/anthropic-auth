...
## CONVENTIONS
- **Canonical Arcus Dist Standard (`dist/<sequence>/<package>/<version>/`)**: All Arcus packaging artifacts are strictly output to `dist/<sequence>/<package_id>/<version>/`. Sequence is the true immutable timeline (strictly monotonic, > current, never resetting to 1). For multi-component suites, the lumped sequence must be the MAX across all components in the suite + 1, ensuring a unified compatibility lock across all suite packages. Version is a display-only artifact. Flat dumps into repository roots or `dist-arcus/` are prohibited.
...
- **Project Packaging Driver**: `scripts/pack-arcus.sh` is anthropic-auth's specialized opencode-plugin pack driver that builds workspace packages, stages runtime assets (`dist`, `src/tui.tsx`, `src/tui-compiled`, `src/rpc`), generates v3 envelopes into `dist/<sequence>/<package>/<version>/releases/`, and auto-allocates sequences against the local Arcus catalog or live gateway.
...
## ANTI-PATTERNS
- **NO Flat Dist Dumps**: Never dump release artifacts directly into repo root or flat `dist-arcus/`; use `dist/<sequence>/<package>/<version>/`.
