...
- Purpose: Prevents Fable 5.1 from rejecting replayed thinking whose original conversation prefix changed after host compaction or prompt transformation
- Location: `packages/core/src/thinking-binding.ts`
- Pattern: Shared outgoing-body inspection recognizes non-empty signed `thinking` and `redacted_thinking` blocks. Only OAuth Fable 5.1 continuations with such replayable history receive `thinking.block_binding.prefix_mismatch_behavior = "drop_block"` and beta `thinking-binding-controls-2026-08-01`; first turns, Mythos 5.1, older models, and API-key routes remain unchanged.
...
**rewriteRequestBody:**
- Purpose: Full Anthropic request body transform — handle cache strategies (including legacy model-specific standby bridges), system sanitization, trailing whitespace tool prefill stripping, tool name prefixing, Claude Code 2.1.258 billing headers and session suffix pinning, conditional Fable 5.1 thinking-prefix recovery, server-side fallback opt-in/marker restoration, and final-body signing
