...
- Pattern: The top-level `output_config.effort` remains anchored to the earliest retained effort. Later changes become empty `role: "system"` messages immediately before their user/tool-result boundary. OAuth Fable 5.1 requests always include `mid-conversation-output-config-2026-07-01`; other models and API-key routes do not. OpenCode carries the history in a bounded request-scoped internal header that is stripped before dispatch, while Pi rebuilds it from active compaction-aware session entries on each turn.
