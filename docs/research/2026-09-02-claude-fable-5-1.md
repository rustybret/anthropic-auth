...
A live OpenCode 1.18.25 session exposed a host-boundary flaw in the initial OpenCode adapter: the plugin recorded effort changes by assistant-record count, but OpenCode lowered 413 pre-transition assistant records into only 160 Anthropic assistant messages. The request therefore sent top-level `xhigh` with no transition marker. OpenCode now tags the actual effort-changing user records with stable checksum-bound text markers before host lowering. The messages hook records the expected transition timeline in a bounded request tracker, and `chat.headers` attaches a fixed-size internal plan only after every message transform has run. When transitions exist, a checksum-bound marker at the current user boundary authenticates that downstream prefix compaction retained the same request lineage. The fetch transform accepts only an exact suffix of the original transition sequence, folds any consumed prefix into the top-level baseline, inserts system effort messages at surviving user boundaries, and rejects holes, reorderings, mutations, or missing/mismatched anchors. It then removes all markers and the internal header before cache lookback, billing suffix derivation, dumps, `cch` signing, and transport.

The first nonce-based implementation failed immediately when used with Magic Context in session `ses_331acff95fferWZOYF1pG0cjOn`: anthropic-auth's transform ran first, then Magic Context tagged and persisted the complete marker text by message ID and part index. On the next pass, that persisted source restored the previous random nonce after anthropic-auth had already replanned, producing `Fable 5.1 effort marker nonce mismatch`. Stable boundary identity plus the post-transform request-plan header removes that ordering dependency. A later live failure in the same session established a second boundary condition: Magic Context legitimately trimmed 382 visible prefix messages, including both historical effort boundaries, after anthropic-auth had planned two transitions. Exact replay over the live 1,230-message input retained the authenticated current-user anchor, accepted zero surviving transitions as the exact empty suffix, folded the final `high` effort into top-level `output_config`, and leaked no internal markers. Without the anchor, the same zero-marker request remains fail-closed.
