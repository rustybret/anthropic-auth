...
A live OpenCode 1.18.25 session exposed a host-boundary flaw in the initial OpenCode adapter: the plugin recorded effort changes by assistant-record count, but OpenCode lowered 413 pre-transition assistant records into only 160 Anthropic assistant messages. The request therefore sent top-level `xhigh` with no transition marker. OpenCode now tags the actual effort-changing user records with stable checksum-bound text markers before host lowering. The messages hook records a digest of the expected marker sequence in a bounded request tracker, and `chat.headers` attaches that fixed-size internal plan only after every message transform has run. The fetch transform validates and removes the markers and internal header, inserts system effort messages at the surviving user boundaries, and fails locally if marker counts, scopes, boundaries, or request-plan digests do not correlate. Marker text is gone before cache lookback, billing suffix derivation, dumps, `cch` signing, and transport.

The first nonce-based implementation failed immediately when used with Magic Context in session `ses_331acff95fferWZOYF1pG0cjOn`: anthropic-auth's transform ran first, then Magic Context tagged and persisted the complete marker text by message ID and part index. On the next pass, that persisted source restored the previous random nonce after anthropic-auth had already replanned, producing `Fable 5.1 effort marker nonce mismatch`. Stable boundary identity plus the post-transform request-plan header removes that ordering dependency. A replay over the exact 375-record compacted live context survived two persisted downstream tagging passes, inserted both `medium` and `high` transitions each time, and leaked no internal markers.
