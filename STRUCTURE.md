...
- Key files: `build-tui.ts` (compiles the SolidJS/OpenTUI files to `src/tui-compiled/` with virtual modules), `smoke-tui-pack-install.ts` (validates TUI installation and dependency resolution), `check-bundle-globals.ts` (validates bundle artifact sizing, verifies the RPC server registry, and guards against stale globals)
...
**Tests:**
- `packages/core/src/tests/`: Core-only unit tests (dump, killswitch, models, prime, quota surfaces, accounts persistence, custom headers, model remap, claustrum, mid-conversation output config, thinking binding)
