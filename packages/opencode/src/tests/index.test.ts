  ...
  test.serial(
    ...
    async () => {
      ...
      try {
        ...
        for (let attempt = 0; attempt < 50; attempt++) {
          const state = await readFile(accountStatePath, 'utf8')
          if (!state.includes(legacyHandle)) break
          await Bun.sleep(10)
        }
    ...
    async () => {
      ...
      try {
        for (let attempt = 0; attempt < 50; attempt++) {
          ...
          if (content.providers?.[0]?.accounts?.length > 0) break
          await Bun.sleep(10)
        }
        expect(
          JSON.parse(await readFile(manifestPath, 'utf8')).providers[0]
        ...
        for (let attempt = 0; attempt < 50; attempt++) {
          ...
          if (!authFile.includes(legacyHandle)) break
          await Bun.sleep(10)
        }
        expect(
